...
export const Card = ({hading, subTitle, paraGraph, buttonList, link, image}) => {
    return (
        <CardContainer className="inter-var">
            <CardBody className="bg-gray-50 relative group/card  dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] dark:bg-black dark:border-white/[0.2] border-black/[0.1] sm:w-[30rem] h-auto rounded-xl p-6 border">
                ...
                <CardItem translateZ="100" className="w-full mt-4 flex flex-wrap gap-3">
                    {buttonList.map((item, idx) => (
                        <Chip color={"primary"} variant={"dot"} key={idx}>{item}</Chip>
                    ))}
                </CardItem>
